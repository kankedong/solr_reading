...
public class SolrResourceLoader implements ResourceLoader,Closeable
{
  ...
  {
    //instanceDir 在设置solr.solr.home的时候是这个路径
    if( instanceDir == null ) {
    ...
    //创建一个classLoader用于加载特定目录的jar包里的类
    this.classLoader = createClassLoader(null, parent);
    
    //增添指定路径的classLoader?
    addToClassLoader("./lib/", null, true);
    
    //重新加载Lucene的Service provider interface，目前看主要包括两部分内容，一是索引格式类，二是分析器类，但重载的目的是引入自定义的类吗？这个还不清楚
    reloadLuceneSPI();
    
    //core的配置属性，目前看不是从构造方法传入的。

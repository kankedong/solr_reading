...
public final class UpdateRequestProcessorChain implements PluginInfoInitialized
{
                                                ...
                                                SolrQueryResponse rsp) 
  {
    ...
    for (int i = chain.length-1; i>=0; i--) {// 从后往前迭代构造处理链 
      ...
      if (skipToDistrib) {
        if (afterDistrib) {
          ...
          // 请求转发的就不需要记录更新日志,忽略LogUpdateProcessor
          continue;
        }
      }
      
      //构造处理链,将下个的UpdateRequestProcessor通过构造函数赋值给当前的next 当next为null 代表处理完毕
      //UpdateRequestProcessor 是一个类似单向链表的数据结构
      processor = factory.getInstance(req, rsp, last);  
      last = processor == null ? last : processor;//如果processor为null那么last保留原值
    }

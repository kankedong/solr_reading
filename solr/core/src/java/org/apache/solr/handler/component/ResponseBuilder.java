...
public class ResponseBuilder
{
  ...
  public boolean doHighlights; //是否高亮
  public boolean doFacets;     //是否分片
  public boolean doStats;      //是否统计
  public boolean doTerms;      //是否？

  private boolean needDocList = false;//是否在结果中放入无序的doc集合
  private boolean needDocSet = false;//是否在结果中放入有序doc集合
  ...
  private SortSpec sortSpec = null; //排序规则
  private GroupingSpecification groupingSpec;
  //used for handling deep paging
  private ScoreDoc scoreDoc;//一个包含得分，和doc的id的对象，但用途是什么？还不清楚
  ...
  private Query highlightQuery = null;//高亮检索的query

  public List<SearchComponent> components;//全部组件
  ...
  public static final String FIELD_SORT_VALUES = "fsv";//分布式检索的参数
  public static final String SHARDS = "shards";//分布式检索的参数，分片的信息
  public static final String IDS = "ids";//lucene的id
  ...
  public static int STAGE_START = 0;//分布式处理，开始阶段
  public static int STAGE_PARSE_QUERY = 1000;//分布式处理，解析查询阶段
  public static int STAGE_TOP_GROUPS = 1500;//分布式处理，top分组阶段
  public static int STAGE_EXECUTE_QUERY = 2000;//分布式处理，执行检索阶段
  public static int STAGE_GET_FIELDS = 3000;//分布式处理，获取field阶段
  public static int STAGE_DONE = Integer.MAX_VALUE;//分布式处理，结束阶段

  public int stage;  // What stage is this current request at?，当前的检索所在的处理过程

  //The address of the Shard
  boolean isDistrib; // is this a distributed search?//这是否是

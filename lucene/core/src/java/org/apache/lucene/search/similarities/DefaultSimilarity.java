...
public class DefaultSimilarity extends TFIDFSimilarity {
  ...
  private static final float[] NORM_TABLE = new float[256];//存放解压的norm值的数组，
   ...
   *  @lucene.experimental */
  /**
   * field长度标准化，field的长度，是基于词条个数进行计算的
   */
  @Override
  public float lengthNorm(FieldInvertState state) {
    final int numTerms;
    if (discountOverlaps)
      numTerms = state.getLength() - state.getNumOverlap();//有重复的term?
    else
      numTerms = state.getLength();
   return state.getBoost() * ((float) (1.0 / Math.sqrt(numTerms)));//没有重复的term?
  }

  /** Implemented as <code>sqrt(freq)</code>. */
  @Override
  public float tf(float freq) {
    return (float)Math.sqrt(freq);//求tf的值
  }
